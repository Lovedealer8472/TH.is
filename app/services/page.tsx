...
import { SERVICE_PAGES, getServicePagePath } from '@/lib/service-pages';

const CARD_SLUGS: Record<string, string | undefined> = {
  repairs: 'tolvuvidgerdir',
  custom: 'sersmidid-bordtolva',
  consulting: 'taekniaadstod-fyrirtaeki',
  installations: 'gagnaflutningur',
  battery: 'fartolvuvidgerdir',
  video: 'vhs-minidv',
};

const CARD_KEYS = ['repairs', 'custom', 'consulting', 'installations', 'battery', 'video'] as const;
    ...
    <div className="py-16 sm:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-16">
          {CARD_KEYS.map((key) => {
            const card = t.services.cards[key];
            const slug = CARD_SLUGS[key];
            return (
              <ServiceCard
                key={key}
                title={card.title}
                ...
                icon={CARD_ICONS[key]}
                className={key === 'battery' ? 'lg:col-start-2' : undefined}
              />
            );
          })}
        </div>
